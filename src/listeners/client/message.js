	...
	async run(message) {
		...
		servers.findOne({
			...
			async function runCommand(client) {
				...
				if(message.guild.id != '402408107851907073') {
				...
				if (cmd.voteLocked == true && await premium(message.author.id, 'Premium') == false && await premium(message.author.id, 'Pro') == false) {
					...
					if (!voted) {
						const voteEmbed = new Discord.MessageEmbed()
							.setDescription('You must **vote** to use this command. **You can vote [here](https://top.gg/bot/740108641054621696/vote)**.')
